...
개념
  ...
  - 송신자가 하나의 세그먼트를 전송한 뒤, 수신자로부터 해당 세그먼트의 ACK를 받을 때까지 기다린 다음 다음 세그먼트를 전송하는 방식입니다. 즉, 한 번에 하나씩만 전송합니다.
...
하지만 전송 중 중간의 한 세그먼트가 손실되면, 그 손실된 지점부터 이후 세그먼트를 모두 다시 보내는 방식입니다.
...
ACK가 도착할 때마다 이 윈도우는 오른쪽으로 한 칸씩 슬라이드하며 새로운 세그먼트 전송이 허용됩니다.
...
즉, 수신자의 버퍼 상황에 따라 동적으로 전송량을 조절하는 것이 Sliding Window의 핵심입니다.
...
전송 계층은 네트워크 통신의 품질을 책임지는 계층입니다.
