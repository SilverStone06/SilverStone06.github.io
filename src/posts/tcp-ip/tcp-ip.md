...
복잡하게 들어가기보단, OSI가 개념이라면, TCP/IP는 실제 인터넷에서 굴러가는 구조라는 정도만 잡고 어떤 식으로 계층이 합쳐지는지 흐름만 살펴보면 훨씬 쉽게 이해할 수 있습니다. 자세한 설명은 해당되는 OSI 계층을 보시면 도움이 됩니다.
...
실제 프로그램 구분은 전송 계층 TCP/UDP의 포트 번호로 이루어진다.
...
TCP/IP는 OSI보다 실제 네트워크에 훨씬 가깝고 현실적인 모델입니다.
